...
class RetentionTests(unittest.TestCase):
    ...
    def test_partial_resume_only_retries_failed_job_then_retires_checkpoints(self):
        with TemporaryDirectory() as d, ExitStack() as stack:
            ...
            self.assertEqual(first['status'],'partial');self.assertEqual(len(calls),6)
            self.assertNotIn('era5_samples.parquet',[v['path'] for v in first['outputs']])
            self.assertFalse(any(name=='era5' for name,_ in calls))
            self.assertFalse(any(name in ('acs','tiger') for name,_ in calls))
            self.assertFalse({'acs_tracts.parquet','tract_boundaries.parquet','tornado_tracts.parquet'} & {v['path'] for v in first['outputs']})
            ...
            self.assertEqual(second['status'],'complete');self.assertEqual(len(calls),7)
